...
@public
def InsertParts(name: bytes32,quantity: uint256,ids: uint256,price: uint256):
...
@public
@constant
def ShowQuantity(part_name: bytes32) -> uint256:
  return self.parts_mapping[part_name]

@public
@constant
def AvailablePartAmount(name_of_part: bytes32, how_many: uint256) -> uint256:
  return how_many*self.price_mapping[name_of_part]

@public
@payable
def UsingOEMParts(name_of_part: bytes32,how_many: uint256,id_of_item: bytes32):
...
@public
@constant
def CalcPoolMoney() -> uint256(wei):
  return self.balance

@public
def ReturnDamagedItems(_name_of_part: bytes32, no_of_pieces: uint256(wei)):
...
@public
def PartToAuto(partid: uint256,vehicle_n: bytes32,_vehicle_id: uint256):
  assert self.auto_industry[msg.sender].typ ==2
...
@public
@constant
def CheckPartPlace(partid: uint256) -> uint256:
  return self.vehicle[partid].vehicle_id

@public
@constant
def CheckPartPlace2(partid: uint256) -> bytes32:
  return self.vehicle[partid].vehicle_name


@public
def AutoAssembled(vehicle_id: uint256):
